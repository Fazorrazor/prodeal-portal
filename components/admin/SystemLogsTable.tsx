        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-brand-border/60">
              <th className="py-4 pr-4 text-[10px] font-bold text-brand-deep-blue/60 uppercase tracking-widest align-bottom">Context</th>
              <th className="px-4 py-4 text-[10px] font-bold text-brand-deep-blue/60 uppercase tracking-widest align-bottom">Message</th>
              <th className="px-4 py-4 text-[10px] font-bold text-brand-deep-blue/60 uppercase tracking-widest align-bottom">Metadata</th>
              <th className="py-4 pl-4 text-[10px] font-bold text-brand-deep-blue/60 uppercase tracking-widest text-right align-bottom">Timestamp</th>
            </tr>
          </thead>
            ...
            {logs.map((log: { id: string, context: string, error_message: string, error_stack: string | null, metadata: Record<string, unknown>, created_at: string }, i: number) => (
              ...
              >
                ...
                <td className="px-4 py-4 text-sm font-semibold text-brand-deep-blue align-top max-w-sm">
                  ...
                  {log.error_stack && (
                    <details className="text-[10px] text-brand-deep-blue/60 font-mono mt-1 cursor-pointer">
                ...
                <td className="px-4 py-4 align-top">
                  {log.metadata && typeof log.metadata === 'object' && Object.keys(log.metadata).length > 0 ? (
                    <div className="text-[10px] font-mono text-brand-deep-blue/60 bg-brand-deep-blue/5 p-2 border-l border-brand-deep-blue/20 max-w-[200px] break-words">
                      <pre className="whitespace-pre-wrap">{JSON.stringify(log.metadata, null, 2)}</pre>
                    </div>
                  ) : (
                    <span className="text-[10px] text-brand-deep-blue/40 uppercase tracking-widest">None</span>
                  )}
                </td>
                <td className="py-4 pl-4 text-brand-deep-blue/60 text-xs font-mono text-right align-top whitespace-nowrap">
