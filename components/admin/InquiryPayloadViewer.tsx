...
export function InquiryPayloadViewer({ payload }: { payload: { productId?: string; productName?: string; [key: string]: unknown } | null }) {
  ...
  return (
    <section>
      <h3 className="text-[10px] font-bold uppercase tracking-widest text-brand-deep-blue/80 mb-4 pb-2 relative">
      ...
      {productName && (
        <div className="mb-8 p-4 bg-black/5 border-l-2 border-brand-blue animate-in fade-in slide-in-from-bottom-2 duration-300">
          <span className="block text-[10px] uppercase tracking-widest text-brand-deep-blue/80 mb-1">Target Product</span>
          <span className="block text-xl font-heading font-bold text-brand-deep-blue uppercase tracking-tight">
            {productName}
          </span>
          {productId && (
            <span className="block text-xs font-mono text-brand-deep-blue/80 mt-1">
              ID: {productId}
            </span>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4">
        {Object.entries(restPayload).map(([key, value], i) => {
          ...
          return (
            ...
            >
              <span className="block text-[10px] uppercase tracking-widest text-brand-deep-blue/80 mb-1">{formattedKey}</span>
